...
export type Run = { run_id: string; status: string; sequence: number; body_markdown: string; citations: Citation[]; progress?: string; error?: string; web_disabled?: boolean; web_activity?: { search: string; reason: string; results?: number; error?: string; pages: { status: string; error?: string }[] }; continuation?: { from_run_id: string; kind: string }; input_scope?: { mode: "quick_qa" | "investigation"; allow_web: boolean; resource_restrictions: string[]; attachment_refs: string[] }; strategy?: string; model_origin?: string; round?: number; active_tool?: string; scope_summary?: { goals: string[]; constraints: string[]; missing: string[]; allow_web: boolean }; budget?: { model_calls: number; tools: number; searches?: number; pages?: number; settled_tokens: number; unreconciled_calls: number } }
