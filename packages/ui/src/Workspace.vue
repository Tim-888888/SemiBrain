...
async function send() {
  ...
  try {
    ...
    if (!pending || pending.payload.text !== text.value || pending.payload.mode !== mode.value || pending.payload.allow_web !== (mode.value === 'investigation' && allowWeb.value) || JSON.stringify(pending.payload.resource_restrictions) !== JSON.stringify(selectedDocuments.value) || JSON.stringify(pending.payload.attachment_refs) !== JSON.stringify(attached.value) || pending.conversationId !== conversation.value.id) pending = {
      conversationId: conversation.value.id, payload: { request_id: crypto.randomUUID(), expected_revision: conversation.value.revision,
        text: text.value, mode: mode.value, continuation_of: messages.value.at(-1)?.status === 'waiting_input' ? messages.value.at(-1)?.run_id : null, resource_restrictions: [...selectedDocuments.value], attachment_refs: [...attached.value], allow_web: mode.value === 'investigation' && allowWeb.value },
    }
