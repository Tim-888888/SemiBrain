...
const running = computed(() => sending.value || (!!activeRun.value && !['succeeded', 'failed', 'partial', 'cancelled'].includes(activeRun.value.status)))
...
function newChat() { generation++; closeStream(); conversation.value = null; messages.value = []; activeRun.value = null; error.value = ''; pending = null; selectedDocuments.value = []; attached.value = []; messagesCursor.value = null; view.value = 'chat' }
...
function subscribe(runId: string, current = generation) {
  ...
  source.addEventListener('snapshot', async event => {
    ...
    activeRun.value = state; await bottom()
    if (current !== generation) return
    if (['succeeded', 'partial', 'failed', 'cancelled'].includes(state.status)) {
      ...
      activeRun.value = null; await loadLists()
    }
  })
...
async function send() {
  ...
  try {
    ...
    if (!pending || pending.payload.text !== text.value || pending.conversationId !== conversation.value.id) pending = {
      conversationId: conversation.value.id, payload: { request_id: crypto.randomUUID(), expected_revision: conversation.value.revision,
        text: text.value, mode: 'quick_qa', resource_restrictions: selectedDocuments.value, attachment_refs: [...attached.value], allow_web: false },
    }
    ...
    <main class="main-panel"><header class="topbar"><span>{{ view === 'chat' ? conversation?.title || '对话工作台' : view === 'knowledge' ? '知识库' : '用户管理' }}</span><span class="workspace-tag">半导体知识空间</span></header>
      ...
      <template v-else>
        <div ref="scrollArea" class="conversation-scroll">
          ...
          <div class="message-column"><button v-if="messagesCursor" class="text-button" @click="olderMessages">加载更早消息</button><section v-for="message in messages" :key="message.id" :class="['message', message.role]"><div v-if="message.role === 'user'" class="question-bubble">{{ message.text }}</div><template v-else><div class="assistant-label"><span class="mini-brand">S</span> SemiBrain<span v-if="message.status === 'failed'" class="muted">处理未完成</span></div><MarkdownAnswer :text="message.body_markdown || (message.status === 'failed' ? '本次处理未完成，请稍后重试。' : '')" :citations="message.citations" /></template></section>
            <section v-if="activeRun" class="message assistant"><div class="assistant-label"><span class="mini-brand">S</span> SemiBrain</div><div class="run-progress" role="status"><span class="pulse-dot"></span>{{ activeRun.progress || '正在处理' }}</div><MarkdownAnswer :text="activeRun.body_markdown" :citations="activeRun.citations" streaming /></section>
          </div>
        </div>
        <div class="composer-area"><p v-if="error" class="error" role="alert">{{ error }}</p><div class="composer"><textarea v-model="text" :disabled="running" placeholder="向 SemiBrain 提问…" aria-label="问题输入框" rows="2" @keydown="keydown"></textarea><div class="composer-toolbar"><select aria-label="问答模式"><option>快速问答</option><option disabled>智能调查 · 尚未开放</option></select><details ref="sourceMenu" class="source-select"><summary>▤ 资料范围{{ selectedDocuments.length ? ` · ${selectedDocuments.length}` : '' }}</summary><div class="source-menu"><p class="small muted">不勾选时检索全部可访问资料</p><label v-for="doc in knowledge" :key="doc.id" class="checkbox"><input v-model="selectedDocuments" type="checkbox" :value="doc.id" />{{ doc.title }}</label><p class="small muted">将已发布文件作为本轮阅读对象</p><label v-for="doc in knowledge.filter(item => item.asset_id)" :key="doc.asset_id" class="checkbox"><input v-model="attached" type="checkbox" :value="doc.asset_id" />阅读：{{ doc.title }}</label><p v-if="!knowledge.length" class="small muted">暂无已发布资料</p></div></details><button class="send-button" :disabled="!text.trim() || running" @click="send" aria-label="发送问题">↑</button></div></div><p class="composer-note">回答可通过引用核验 · 业务数据为合成演示数据 · Enter 发送，Shift + Enter 换行</p></div>
      </template>
    </main>
